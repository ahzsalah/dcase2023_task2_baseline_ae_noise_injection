...
# Newly Added Code
# function adds white Noise to data
# def add_noise(data, noise_level=0.2):
#     noise = torch.randn_like(data) * noise_level
#     return data + noise


def plot_signals(original, noisy, title1='Original Signal', title2='Noisy Signal'):
...
def calculate_snr(original_data, noisy_data):
    # Calculate signal power
    signal_power = torch.var(original_data)
    
    # Calculate noise power
    noise = noisy_data - original_data
    noise_power = torch.var(noise) if torch.var(noise) > 1e-10 else 1e-10
    snr = 10 * torch.log10(signal_power / noise_power)

    return snr.item()

# def apply_shifting(data, shift_max):
#     # Shift the data
#     shift_amount = np.random.randint(-shift_max, shift_max)
#     data = torch.roll(data, shifts=shift_amount, dims=1)
    
#     # Ensure that the rolled part does not wrap around
#     if shift_amount > 0:
#         data[:, :shift_amount] = 0
#     else:
#         data[:, shift_amount:] = 0
...
# def frequency_mask(data, mask_percentage):
#     num_freqs = data.shape[1]
#     num_mask = int(num_freqs * mask_percentage)
#     num_mask = max(1, num_mask)
#     mask_start = random.randint(0, num_freqs - num_mask)
#     data[:, mask_start:mask_start + num_mask] = 0
#     return data

def generate_pink_noise(data, noise_level=0.25):
    num_samples = data.shape[1]
    white_noise = torch.randn_like(data)
    
    # Generating the frequencies
    frequencies = torch.fft.rfftfreq(num_samples, d=1.0)
    frequencies[0] = frequencies[1]

    # pink noise filter (1/f)
    pink_filter = 1 / torch.sqrt(frequencies)
    # Applying the filter in the frequency domain
    pink_noise_fft = torch.fft.rfft(white_noise) * pink_filter.unsqueeze(0)
    # convert back to time domain
    pink_noise = torch.fft.irfft(pink_noise_fft, n=num_samples)
    # Normalize and scale the pink noise
    pink_noise = pink_noise / pink_noise.std() * noise_level
    
    return data + pink_noise    


# def add_background_noise(data, noise_files, noise_level=0.1):
#     noise_file = random.choice(noise_files)
#     noise, _ = librosa.load(noise_file, sr=None)
#     if len(noise) < data.shape[0]:
#         repeat_factor = (data.shape[0] // len(noise)) + 1
#         noise = np.tile(noise, repeat_factor)
#     noise = noise[:data.shape[0]]
#     noise = torch.from_numpy(noise).to(data.device).float()
#     data_noisy = data + noise * noise_level
#     return data_noisy

# def apply_frequency_mask(data, mask_percentage=0.1, dim=1):
#     num_freqs = data.shape[dim]
#     num_mask = int(num_freqs * mask_percentage)
#     num_mask = max(1, num_mask)
#     mask_start = torch.randint(0, num_freqs - num_mask, (1,)).item()
#     mask = torch.ones(data.shape[dim], device=data.device)
#     mask[mask_start:mask_start + num_mask] = 0
#     mask = mask.view([-1 if i == dim else 1 for i in range(data.ndim)]).expand_as(data)
#     data_masked = data * mask

#     return data_masked

# def generate_brown_noise(data, noise_level=0.1):
#     num_samples = data.shape[1]
#     white_noise = torch.randn_like(data)
#     frequencies = torch.fft.rfftfreq(num_samples, d=1.0)
...
#     brown_filter = 1 / (frequencies ** 2)
    
#     brown_noise_fft = torch.fft.rfft(white_noise) * brown_filter.unsqueeze(0)
    
#     brown_noise = torch.fft.irfft(brown_noise_fft, n=num_samples)
    
#     brown_noise = brown_noise / brown_noise.std() * noise_level
    
#     return data + brown_noise


# def apply_fft(data):
#     data_fft = torch.fft.rfft(data, dim=1)
#     data_fft = torch.abs(data_fft)
#     return data_fft
...
class DCASE2023T2AE(BaseModel):
    ...
    def train(self, epoch):
        ...
        for batch_idx, batch in enumerate(tqdm(train_loader)):
            ...
            # Calculate and store SNR before adding noise
            # original_data = data.clone()  # Clone the original data for comparison
            # snr_before = calculate_snr(original_data, original_data)
            # snrs_before_noise.append(snr_before)
            # print(f"SNR for batch before noise {batch_idx}: {snr_before} dB")
            
            # Apply pink noise
            data = generate_pink_noise(data, noise_level=0.25)

            # Calculate and store SNR after adding noise
            # snr_after = calculate_snr(original_data, data_noisy)
            # snrs_after_noise.append(snr_after)
            # print(f"SNR for batch after noise {batch_idx}: {snr_after} dB")


            # print("Variance of original data:", torch.var(original_data))
            # print("Variance of noise (noisy data - original data):", torch.var(data - original_data))

            # Convert data from PyTorch tensor to NumPy array for plotting (if necessary)
            # original_data_np = original_data.cpu().numpy()
            # noisy_data_np = data.cpu().numpy()

            # Plotting the original and noisy signal
            # plot_signals(original_data_np, noisy_data_np, title1='Original Signal', title2='Signal with Pink Noise')
    ...
    # Plotting SNR before and after adding noise
        # plt.figure(figsize=(10, 6))
        # plt.plot(snrs_before_noise, label='SNR Before Noise', marker='o')
        # plt.plot(snrs_after_noise, label='SNR After Noise', marker='x')
        # plt.xlabel('Batch Index')
        # plt.ylabel('SNR (dB)')
        # plt.title('SNR Before and After Adding Pink Noise')
        # plt.legend()
        # plt.show()
